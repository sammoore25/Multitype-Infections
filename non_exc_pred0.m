...
Tmat=[p11/(p11+rho),p21/(p21+rho),p31/(p31+rho);p12/(p12+rho),p22/(p22+rho),p32/(p32+rho);p13/(p13+rho),p23/(p23+rho),p33/(p33+rho)];
     ...
     for k=2:numt+1
         ...
         for j=1:hsize
             H1(j,k)=1-Tmat(1,1)*(1-prod(H1(Neighs{j},k-1)))-Tmat(1,2)*(1-prod(H2(Neighs{j},k-1)))-Tmat(1,3)*(1-prod(H3(Neighs{j},k-1)));
             H2(j,k)=1-Tmat(2,1)*(1-prod(H1(Neighs{j},k-1)))-Tmat(2,2)*(1-prod(H2(Neighs{j},k-1)))-Tmat(2,3)*(1-prod(H3(Neighs{j},k-1)));
             H3(j,k)=1-Tmat(3,1)*(1-prod(H1(Neighs{j},k-1)))-Tmat(3,2)*(1-prod(H2(Neighs{j},k-1)))-Tmat(3,3)*(1-prod(H3(Neighs{j},k-1)));
...
for i=1:n
         V1(i)=(1-prod(H1(Neighs2{i},end)));
         V2(i)=(1-prod(H2(Neighs2{i},end)));
          V3(i)=(1-prod(H3(Neighs2{i},end)));
end
